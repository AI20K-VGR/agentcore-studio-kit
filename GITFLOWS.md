...
thành **1 repo cha (workspace) + 6 repo con (submodule)**. Đọc kỹ phần "Pitfalls" ở cuối — 90%
...
## 1. Bản đồ repo (7 repo)

| Path trong workspace | Repo GitHub (private) | Owner (write) | Nội dung |
|---|---|---|---|
| **(gốc)** `agentcore-studio-kit` | `hieubui2409/agentcore-studio-kit` | **mentor** | Repo CHA = workspace root: `pyproject.toml` + `uv.lock` + `docker/` + `docker-compose*.yml` + `.github/` CI + `Makefile` + `conftest.py` + `tests/` + `apps/web/` + `scripts/` + `docs/` + `README.md`. Ghim con trỏ 6 submodule. |
...
├── pyproject.toml · uv.lock · docker/ · .github/ · Makefile · conftest.py · tests/ · apps/web/   ← ở repo cha
...
make setup                      # = uv sync (cần đủ 6 submodule đã init)
...
- Mỗi repo con có `.github/workflows/` **riêng** (lint/mypy/test per-package) — chạy độc lập khi push
  repo đó. (Copy từ khung `.github/workflows/ci.yml` ở repo cha, thu gọn về đúng package.)
- Repo **cha** giữ CI **integration** (full workspace + DB + build Docker) — chạy khi bump pointer.
- `leak-test` vẫn là **continue-on-error** (đỏ-by-design) cho tới khi DE làm `kb.search` xanh (xem
  README §CI). NDA-denylist pre-commit nên cài ở **mỗi** repo (chống file mentor/rubric lọt vào bất kỳ
